...
import { Row, Text, Rule, Col, TwoUp, Center, Box } from '@callil/uni-react'
import * as indigo from '@callil/uni-react'
...
const fetchComponentData = (id) => {
  return Promise.all([fetchProperties(id), fetchMetadata(id)]).then(
    ([properties, metadata]) => {
    ...
    <Col expand minHeight="100vh">
      ...
      <Row p="7">
        ...
        >
          <LiveProvider
            scope={indigo}
            code="<Button border>Hello World!</Button>"
          >
            <Col width="50%">
              <LiveEditor />
              <LiveError />
      ...
      <Row>
        <Text bold fontSize="4" p="4" px="7">
          Properties
        </Text>
      </Row>
      {properties !== null ? <Table rows={properties.types} /> : null}
    </Col>
  )
}
